...
namespace Pixeval.Objects.Exceptions.Logger
{
    public class ExceptionDumper
    {
        ...
        {
            var stack = new ApplicationLogStack("Pixeval", ".NET Runtime").GetFirst();
            ...
            sb.AppendLine();
            sb.AppendLine(@"We have encountered an unrecoverable problem. A dumper file with error snapshot has been created.");
            ...
            sb.AppendLine(@"        Represents the latest application event log created by Pixeval");
            sb.AppendLine($"        Creation: {stack.Creation}");
            sb.AppendLine(@"        Data:");
            sb.AppendLine(FormatMultilineData(stack.Data, 12));
            sb.AppendLine(@"    End Event Log");
            sb.AppendLine();
            sb.AppendLine(@"    Begin Exception Log");
            sb.AppendLine(@"        Exception: ");
            sb.AppendLine(FormatMultilineData(exceptionMessage, 12));
